...
function AboutStandards() {
  return (
    <section className="py-10 px-16 bg-dark text-background max-2xl:px-8">
      <div className="flex gap-24 max-2xl:flex-col max-2xl:gap-10">
        <div>
          <Fade triggerOnce>
            <p className="uppercase tracking-wider text-lg text-primary font-bold max-sm:text-sm">
              Standardy i certyfikaty
            </p>
            <h2 className="text-5xl tracking-tight font-semibold max-sm:text-3xl w-max">
              Budujemy zgodnie <br></br>z najwyższymi standardami.
            </h2>
          </Fade>
        </div>
        <div className="flex gap-6 justify-between w-full max-lg:grid max-lg:grid-cols-2 max-lg:gap-10 max-sm:grid-cols-1">
          ...
          <p className="border-r  border-border/50 max-lg:hidden"></p>
